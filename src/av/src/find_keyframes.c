...
void dump_stream_info(const AVStream *stream) {
    ...
    if (!codecpar) {
        fprintf(stderr,
                "dump_stream_info: Stream #%d has no codec parameters\n",
                stream->index);
        return;
    }
    ...
    if (codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        av_channel_layout_describe(&codecpar->ch_layout, ch_layout_str,
...
void dump_program_info(const AVProgram *program) {
    ...
    if (av_dict_count(program->metadata) > 0) {
        fprintf(stderr, "  Metadata:\n");
        while ((tag = av_dict_get(program->metadata, "", tag,
...
static void log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt) {
    ...
    printf("key: "
           "%d\tstream_index:%d\tpts_time:%s\tdts_time:%s\tduration_time:%s\n",
           pkt->flags & AV_PKT_FLAG_KEY, pkt->stream_index,
