...
void log_packet(TranscodeContext *tctx, AVPacket *pkt, const char *tag) {
  ...
  fprintf(stderr, "[%s] key: %d stream_index: %d pts_time: %s duration_time: %s\n", 
...
void dump_transcode_context(TranscodeContext *tctx) {
  ...
  fprintf(stderr, "\tInput video start time: %s\n", 
          av_ts2timestr(tctx->in_video_stream->start_time, &tctx->in_video_stream->time_base));
  fprintf(stderr, "\tInput audio stream index: %d\n", tctx->in_audio_stream_index);
  fprintf(stderr, "\tInput audio start time: %s\n", 
          av_ts2timestr(tctx->in_audio_stream->start_time, &tctx->in_audio_stream->time_base));
...
AVCodecContext* config_dec_ctx(AVStream *stream, AVFormatContext *ifmt_ctx, AVBufferRef *hw_device_ctx) {
  int ret;
  ...
  if (ret < 0) {
    fprintf(stderr, "Failed to copy decoder params to input decoder context");
    return NULL;
  }
...
int config_input(TranscodeContext *tctx) {
  int ret;
  tctx->ifmt_ctx = NULL;
  if ((ret = avformat_open_input(&tctx->ifmt_ctx, tctx->in_filename, 0, 0)) < 0) {
  ...
  if ((ret = av_find_best_stream(tctx->ifmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0)) < 0) {
  ...
  tctx->dec_ctx = config_dec_ctx(tctx->in_video_stream, tctx->ifmt_ctx, tctx->hw_device_ctx);
  if (tctx->dec_ctx == NULL) {
    fprintf(stderr, "Failed to config decoder context for video stream\n");
    return -1;
  }

  if ((ret = av_find_best_stream(tctx->ifmt_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0)) < 0) {
...
int config_output(TranscodeContext *tctx) {
  ...
  const AVOutputFormat *mpegts_ofmt = av_guess_format(NULL, NULL, "video/MP2T");
  if (mpegts_ofmt == NULL) {
    fprintf(stderr, "Failed guessing mpegts format\n");
    return -1;
  }

  if ((ret = avformat_alloc_output_context2(&tctx->ofmt_ctx, mpegts_ofmt, NULL, NULL)) < 0 
      || !tctx->ofmt_ctx) {
  ...
  ret = avcodec_parameters_copy(out_audio_stream->codecpar, tctx->in_audio_stream->codecpar);
  if (ret < 0) {
...
int encode_write(TranscodeContext *tctx, AVPacket *pkt, AVFrame *frame) {
  ...
  while (1) {
    ...
    av_packet_rescale_ts(pkt, enc_ctx->time_base, tctx->out_video_stream->time_base);
...
int dec_enc(TranscodeContext *tctx, AVPacket *pkt, int64_t start_ts, int64_t end_ts) {
  AVCodecContext *enc_ctx = tctx->enc_ctx;
  ...
  while (ret >= 0) {
    ...
    int64_t frame_ts = av_rescale_q(frame->pts, dec_ctx->pkt_timebase, 
                                    av_get_time_base_q());

    if (frame_ts < start_ts) {
      fprintf(stderr, 
              "Decoded frame timestamp %ld is smaller than start timestamp %ld\n",
              frame_ts, start_ts);
      goto dec_enc_end;
    }
    if (end_ts <= frame_ts) {
      fprintf(stderr, 
              "Decoded frame timestamp %ld is bigger than or equal to end timestamp %ld\n",
              frame_ts, end_ts);
      return 0;
    }

    frame->pts = av_rescale_q(frame->pts, dec_ctx->pkt_timebase, 
                              enc_ctx->time_base);
...
/**
 * - seek to start and transcode duration length segment from file of in_filename
...
int transcode_segment(const char *in_filename,
  ...
  if ((ret = av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_QSV, NULL, NULL, 0)) < 0) {
  ...
  int64_t stream_start_ts = av_rescale_q(tctx->in_video_stream->start_time, 
  ...
  avformat_seek_file(tctx->ifmt_ctx, -1, INT64_MIN, start_ts, start_ts, AVSEEK_FLAG_BACKWARD);

  fprintf(stderr, "start: %ld\tend: %ld\n", start_ts, end_ts);
  while (ret >= 0) {
    if ((ret = av_read_frame(tctx->ifmt_ctx, pkt)) < 0) break;

    int64_t pkt_pts = av_rescale_q(pkt->pts, 
                                   tctx->ifmt_ctx->streams[pkt->stream_index]->time_base, 
                                   av_get_time_base_q());
    log_packet(tctx, pkt, "in");
    if (pkt_pts > end_ts) break;
    ...
    if (pkt->stream_index == tctx->in_video_stream_index) {
      ...
      if (pkt_pts < start_ts) continue;
      if (!tctx->enc_ctx->hw_frames_ctx) continue;
      // copy audio codecs
      // continue;
      pkt->stream_index = OUT_AUDIO_STREAM_INDEX;
      av_packet_rescale_ts(pkt, tctx->in_audio_stream->time_base, tctx->out_audio_stream->time_base);
      pkt->pos = -1;
      log_packet(tctx, pkt, "out");
...
int main(int argc, char **argv) {
  ...
  if (ret < 0) {
    fprintf(
      stderr, 
      "failed to transcode segment from %s starting at %lf for %lf seconds", 
